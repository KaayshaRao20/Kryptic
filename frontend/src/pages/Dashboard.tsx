      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-textPrimary tracking-tight">
            {selectedCustomer ? `${selectedCustomer.name}` : 'System Overview'}
          </h1>
          <p className="text-sm text-textSecondary mt-1">
            {selectedCustomer
              ? `Customer Account: ${selectedCustomer.id} (${selectedCustomer.accountType}) • Isolated payment flow Digital Twin`
              : 'Payment Flow Digital Twin - Real-time monitoring'}
          </p>
        </div>
        <div className="flex items-center gap-6 bg-surface px-4 py-2 rounded-lg border border-secondary shadow-sm">
          ...
          <div className="flex flex-col items-end">
            <span className="text-xs text-textSecondary uppercase font-semibold">Customer Health</span>
            <div className="flex items-center gap-2 mt-0.5">
              <div className={cn("h-2 w-2 rounded-full animate-pulse", selectedCustomer?.riskLevel === 'CRITICAL' ? "bg-riskRed" : "bg-healthGreen")} />
              <span className={cn("text-sm font-medium", selectedCustomer?.riskLevel === 'CRITICAL' ? "text-riskRed" : "text-healthGreen")}>
                {selectedCustomer ? (selectedCustomer.riskLevel === 'CRITICAL' ? 'High Risk' : 'Protected') : (metrics.critical > 0 ? 'Degraded' : 'Online')}
              </span>
            </div>
          </div>
        </div>
        ...
        {selectedCustomer ? (
          <>
            <MetricCard
              ...
              value={selectedCustomer.totalTransactions.toLocaleString()}
              subvalue="Isolated customer history"
              icon={<Activity className="text-techBlue w-5 h-5" />}
            />
            <MetricCard
              title="Customer Volume"
              value={`₹${selectedCustomer.totalVolume.toLocaleString()}`}
              subvalue="Cleared total"
              icon={<CreditCard className="text-techBlue w-5 h-5" />}
            />
            <MetricCard
              title="Customer Risk Score"
              value={`${selectedCustomer.riskScore}/100`}
              subvalue={`${selectedCustomer.riskLevel} status`}
              icon={<ShieldCheck className="text-healthGreen w-5 h-5" />}
            />
            <MetricCard
              title="Active Payment Cards"
              value={`${selectedCustomer.activeCards} Cards`}
              subvalue={`Account: ${selectedCustomer.status}`}
              icon={<AlertTriangle className={selectedCustomer.riskLevel === 'CRITICAL' ? "text-riskRed w-5 h-5" : "text-amber-500 w-5 h-5"} />}
            />
          </>
        ) : (
          <>
